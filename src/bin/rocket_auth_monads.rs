...
use diesel::prelude::*;
use dotenv::dotenv;
use rocket::http::{Status, Cookies, Cookie, HeaderMap};
use rocket::Request;
...
use rust_web::models::*;
use rust_web::schema::*;
use crate::LoginError::InvalidData;
...
impl<'a, 'r> FromRequest<'a, 'r> for AuthenticatedUser {
    ...
    fn from_request(request: &'a Request<'r>) -> Outcome<AuthenticatedUser, LoginError> {
        let headers_result = read_auth_from_headers(&request.headers());
        headers_result.and_then(|(u, p)| -> Outcome<AuthenticatedUser, LoginError> {
            let conn_str = local_conn_string();
            let maybe_user = fetch_user_by_email(&conn_str, &String::from(u));
            option_to_outcome(maybe_user, (Status::NotFound, LoginError::UsernameDoesNotExist))
                .and_then(|user: UserEntity| -> Outcome<AuthInfoEntity, LoginError> {
                    let fail = (Status::MovedPermanently, LoginError::WrongPassword);
                    option_to_outcome(fetch_auth_info_by_user_id(&conn_str, user.id), fail)
                })
                .and_then(|auth_info: AuthInfoEntity| -> Outcome<AuthenticatedUser, LoginError> {
                    let hash = hash_password(&String::from(p));
                    if hash == auth_info.password_hash {
                        Outcome::Success(AuthenticatedUser{user_id: auth_info.user_id})
                    } else {
...
fn read_auth_from_headers(headers: &HeaderMap) -> Outcome<(String, String), LoginError> {
    ...
    option_to_outcome(headers.get_one("username"), fail.clone())
        .and_then(|u| -> Outcome<(String, String), LoginError> {
            option_to_outcome(headers.get_one("password"), fail.clone())
                .and_then(|p| -> Outcome<(String, String), LoginError> {
                    Outcome::Success((String::from(u), String::from(p)))
                })
        })
}
...
fn option_to_outcome<R>(result: Option<R>, status_error: (Status, LoginError)) -> Outcome<R, LoginError> {
    match result {
